...
// Returns the high byte from a uint16_t
uint8_t Command_Packet::GetHighByte(uint16_t w)
{
	return (uint8_t)(w>>8)&0x00FF;
}

// Returns the low byte from a uint16_t
...
uint16_t Response_Packet::CalculateChecksum(uint8_t* buffer, int length)
{
	uint16_t checksum = 0;
...
//	203 - Device error
	// Not implemented due to memory restrictions on the arduino
	// may revisit this if I find a need for it
//int FPS_GT511C3::SetTemplate(uint8_t* tmplt, int id, bool duplicateCheck)
//{
	// Not implemented due to memory restrictions on the arduino
	// may revisit this if I find a need for it
	//return -1;
...
Response_Packet* FPS_GT511C3::GetResponse()
{
	...
	while (done == false)
	{
        if ( Serial_IsCharReceived()) {
		    ...
		    firstbyte = (uint8_t)Serial_ReceiveByte();
        }
	...
	for (int i=1; i < 12; i++)
	{
		while (Serial_IsCharReceived() == false) Delay_MS(10);
		resp[i]= (uint8_t) Serial_ReceiveByte();
	}
...
void FPS_GT511C3::SendToSerial(uint8_t data[], int length)
{
  bool first=true;
  Serial_SendString("\"");
  for(int i=0; i<length; i++)
